...
os.chdir(target_dir)
json_path=os.path.join(target_dir, "us_states.json")
...
async def get_property_address(page):
    try:
        ...
        print(f"⚠️ Failed to extract address: {e}")
        return "Unknown"
    ...
    async with async_playwright() as p:
        ...
        for item in results:
            ...
            try:
                ...
                print(f"Failed to scrape address for {resolved_url}: {e}")
                address = "Unknown"
...
async def main():
    ...
    write_deduped_to_csv(deduped_results, filename="grestar_deduped.csv")
