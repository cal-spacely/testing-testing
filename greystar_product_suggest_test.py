...
async def enrich_property(context, item):
    seo_url = construct_seo_url(item)
    page = await context.new_page()
    try:
        await page.goto(seo_url, timeout=15000)
...
async def enrich_with_address_concurrent(properties, batch_size=5):
    enriched = []
    async with async_playwright() as playwright:
        ...
        for i in range(0, len(properties), batch_size):
            ...
            tasks = [enrich_property(context, item) for item in batch]
...
async def main():
    ...
    deduped_results = dedupe_properties(all_results)
    export_scraper_input(deduped_results, data_dir)
    print("...Enriching properties")
    enriched_results = await enrich_with_address_concurrent(deduped_results)
    write_deduped_to_csv(enriched_results, filename="greystar_deduped.csv")
