...
USE_SERPAPI = False  # Set True when SerpAPI quota resets

# Toggle extraction backendc
USE_GEMINI = False   # True = Gemini, False = llama-server
...
Rules:
- If the article does not explicitly state a cause, return "unknown".
- If the article does not explicitly state a location, return "unknown".
...
"""
    ...
    try:
        ...
        # Gemini returns JSON inside JSON — extract safely
        try:
            return json.loads(
